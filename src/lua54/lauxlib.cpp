...
static int skipBOM (LoadS *ls) {
  const char *p = "\xEF\xBB\xBF";  /* UTF-8 BOM mark */
  size_t l = strlen(p);
  if (ls->size == 0) return 0;
  if (ls->size < l) return *ls->s;
  if (memcmp((void *)ls->s, p, strlen(p))) return *ls->s;
  ls->s += l, ls->size -= l;
  return *ls->s;
}
...
static int skipcomment (LoadS *ls) {
  int c = skipBOM(ls);
  const char *e = ls->s + ls->size;
  if (c == '#') {  /* first line is a comment (Unix exec. file)? */
    do {  /* skip first line */
          ++ls->s, --ls->size;
    } while (ls->s < e && (*ls->s) != '\n');
  ...
  if (L->iostream == NULL) {
    ...
    if (filename == NULL) {
      lua_pushliteral(L, "=stdin");
        ...
        lua_pushfstring(L, "@%s", filename);
        status = L->iostream(filename, const_cast<char **>(&ls.s), &ls.size);
        if (status) {
...
LUALIB_API lua_State *luaL_newstate (void) {
  return luaL_newstateex(NULL, NULL, NULL);
}

LUALIB_API lua_State *luaL_newstateex (lua_Alloc c, lua_IOStream s, lua_Readable r) {
  if (c == NULL) {
      c = l_alloc;
  }
  lua_State *L = lua_newstateex(c, NULL, s, r);
  if (L) {
