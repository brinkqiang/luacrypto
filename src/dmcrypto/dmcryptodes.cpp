...
static void DESSetKeySub(uint32_t SK[32], uint8_t key[8])
{
    int32_t  i;
    uint32_t X, Y, T;
    ...
    uint8_t *pacOutput)
{
    ...
    if (nMode == DES_ENCRYPT)
    {
        while (nLength > 0)
        {
            for (i = 0; i < 8; i++)
            {
                pacOutput[i] = (uint8_t)(pacInput[i] ^ pIV->acBlock[i]);
            }
    ...
    else /* DES_DECRYPT */
    {
        while (nLength > 0)
        {
            ...
            for (i = 0; i < 8; i++)
            {
                pacOutput[i] = (uint8_t)(pacOutput[i] ^ pIV->acBlock[i]);
            }
...
static void DES3Setkey(uint32_t esk[96], uint32_t dsk[96], uint8_t  key[24])
{
    int i;
...
//
static const uint8_t des_test_keys[24] =
{
...
static const uint8_t des_test_iv[8] =
{
    0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF,
};

static const uint8_t des_test_buf[8] =
{
    0x4E, 0x6F, 0x77, 0x20, 0x69, 0x73, 0x20, 0x74
};

static const uint8_t des_test_ecb_dec[3][8] =
{
...
static const uint8_t des_test_ecb_enc[3][8] =
{
...
static const uint8_t des_test_cbc_dec[3][8] =
{
...
static const uint8_t des_test_cbc_enc[3][8] =
{
...
int32_t DESSelfTest(int32_t nVerbose)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
        case 0:
            DESSetDecKey(&ctx, (uint8_t *)des_test_keys);
            break;

        case 1:
            DESSetEncKey(&ctx, (uint8_t *)des_test_keys);
            break;
